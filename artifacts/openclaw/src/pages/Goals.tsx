        ...
        <div className="flex justify-between items-start gap-3">
          <div className="min-w-0">
            <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight flex items-center gap-2">
              <Target className="w-6 h-6 md:w-8 md:h-8 text-primary shrink-0" /> Goals
            </h1>
            <p className="text-muted-foreground mt-1 text-xs md:text-sm">Persistent objectives that agents work toward autonomously.</p>
          </div>
          <button onClick={() => setCreating(true)} className="bg-primary hover:bg-primary/90 text-primary-foreground px-3 py-2 md:px-5 md:py-2.5 rounded-xl font-semibold shadow-lg shadow-primary/20 flex items-center gap-1.5 text-sm shrink-0">
            <Plus className="w-4 h-4" /> <span className="hidden sm:inline">New</span> Goal
          </button>
        </div>
      </header>

      <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pb-20 md:pb-8">
        {goals.length === 0 ? (
          ...
          <div key={g.id} className="glass-panel rounded-2xl overflow-hidden">
            <div className="p-3 md:p-4 cursor-pointer select-none hover:bg-white/5" onClick={() => expandGoal(g.id)}>
              {/* Row 1: status + title + expand */}
              <div className="flex items-start gap-2.5">
                <div className="mt-0.5 shrink-0">{statusIcon(g.status)}</div>
                <div className="flex-1 min-w-0">
                  <span className="text-white font-semibold text-sm leading-tight">{g.title}</span>
                </div>
              ...
              <div className="flex items-center gap-2 mt-1.5 ml-6">
                ...
                <div className="flex items-center gap-1"><Bot className="w-3 h-3 text-primary" /><span className="text-[11px] text-white/50">{agentName(g.agentId)}</span></div>
                {g.deadline && <div className="flex items-center gap-1"><Calendar className="w-3 h-3 text-white/30" /><span className="text-[11px] text-white/30">{format(new Date(g.deadline), "MMM d")}</span></div>}
                <span className="text-[11px] text-white/30">{formatDistanceToNow(new Date(g.createdAt), { addSuffix: true })}</span>
              </div>
              ...
              <div className="flex items-center gap-2 mt-2 ml-6">
                ...
                <span className="text-[10px] text-white/40 w-8 text-right shrink-0">{g.progress}%</span>
                <div className="flex items-center gap-0.5 shrink-0">
                  {g.status === "active" && <button onClick={e => { e.stopPropagation(); updateGoal(g.id, { status: "paused" }); }} className="p-1 rounded-lg hover:bg-white/10 text-yellow-400" title="Pause"><Pause className="w-3.5 h-3.5" /></button>}
                  {g.status === "paused" && <button onClick={e => { e.stopPropagation(); updateGoal(g.id, { status: "active" }); }} className="p-1 rounded-lg hover:bg-white/10 text-green-400" title="Resume"><Play className="w-3.5 h-3.5" /></button>}
                  {g.status === "active" && <button onClick={e => { e.stopPropagation(); executeStep(g.id); }} className="p-1 rounded-lg hover:bg-white/10 text-primary" title="Run"><Play className="w-3.5 h-3.5" /></button>}
                  <button onClick={e => { e.stopPropagation(); deleteGoal(g.id); }} className="p-1 rounded-lg hover:bg-white/10 text-red-400/50 hover:text-red-400" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </div>
            </div>
